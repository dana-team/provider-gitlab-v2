...
import (
	...
	apisCluster "github.com/dana-team/provider-gitlab-v2/apis/cluster"
	apisNamespaced "github.com/dana-team/provider-gitlab-v2/apis/namespaced"
	"github.com/dana-team/provider-gitlab-v2/config"
	"github.com/dana-team/provider-gitlab-v2/internal/clients"
	controllerCluster "github.com/dana-team/provider-gitlab-v2/internal/controller/cluster"
	controllerNamespaced "github.com/dana-team/provider-gitlab-v2/internal/controller/namespaced"
	"github.com/dana-team/provider-gitlab-v2/internal/features"
	"github.com/dana-team/provider-gitlab-v2/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for GitLab-v2").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-gitlab-v2"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-gitlab-v2",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped GitLab-v2 APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced GitLab-v2 APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped GitLab-v2 controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced GitLab-v2 controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped GitLab-v2 controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced GitLab-v2 controllers")
	}
